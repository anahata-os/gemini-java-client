 ...
 * <ul>
 *   <li>{@link uno.anahata.ai.tools.ToolManager}: The main orchestrator for tool management. It discovers
 ...
 *
 *   <li>{@link uno.anahata.ai.tools.AIToolMethod}: An annotation used to mark a Java method as a tool
 *       callable by the AI. It includes properties for description and context management behavior.</li>
 *
 *   <li>{@link uno.anahata.ai.tools.AIToolParam}: An annotation for describing the parameters of a tool
 *       method, which is used to generate a more informative schema for the model.</li>
 *
 *   <li>{@link uno.anahata.ai.tools.FunctionPrompter}: An interface that decouples the tool confirmation
 *       process from any specific UI, allowing for flexible implementations (e.g., Swing dialogs, console prompts).</li>
 *
 *   <li>{@link uno.anahata.ai.tools.FailureTracker}: A utility to prevent the model from getting stuck in
 *       a loop of repeatedly calling a failing tool.</li>
 *
 *   <li>{@link uno.anahata.ai.tools.ContextBehavior}: An enum that defines how the result of a tool call
 *       should affect the conversation context (e.g., as an ephemeral event or a stateful resource update).</li>
 * </ul>
 *
 * <h2>Subpackages:</h2>
 * <ul>
 *   <li>{@link uno.anahata.ai.tools.schema}: Contains the logic for generating the JSON schema required
 *       by the Gemini API from Java method signatures.</li>
 *
 *   <li>{@link uno.anahata.ai.tools.spi}: Provides a Service Provider Interface with a set of pre-built,
